...
const { createCaseForAlert, addAlertToCase } = require('../src/services/caseService');
...
describe('createCaseForAlert', () => {
  ...
  test('if nothing attaches at all it throws, but names the orphaned case', async () => {
    // Moved down from the createCaseForGroup block: the property is about
    // createCaseFromAlerts, which both entry points share, and this is now the
    // only route to it from a test
    const client = fakeClient();
    client.attachAlert.mockRejectedValue(httpError(500, { message: 'nope' }));
    await expect(createCaseForAlert('api-key', 'alert-1')).rejects.toThrow(/case-1/);
  });

  test('a non-default space is carried into the case link', async () => {
    fakeClient({ getAlertById: jest.fn().mockResolvedValue(alert('a1', { spaceId: 'soc' })) });
    const result = await createCaseForAlert('api-key', 'a1');
